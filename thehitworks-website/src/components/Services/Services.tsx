      ...
      <section className="bg-milk py-20">
        <div className="max-w-4xl mx-auto px-6 flex flex-col gap-4">
          {SERVICES.map((s) => {
            ...
            return (
              <div key={s.id} className={`bg-paper border-2 border-ink pixel-corners ${open ? "" : ""}`}>
                <button onClick={() => setOpenId(open ? null : s.id)} className="w-full flex items-center gap-4 p-5 text-left">
                  ...
                  <div className="flex-1">
                    <div className="font-term text-2xl text-ink">{s.title}</div>
                    <div className="text-ink-70 text-base">{s.tagline}</div>
                  </div>
                ...
                {open ? (
                  <div className="px-5 pb-6 pt-1 border-t-2 border-milk-deep grid md:grid-cols-2 gap-6">
                    <div>
                      <p className="text-ink-70 text-lg mb-4">{s.overview}</p>
                      <div className="font-pixel text-10px text-royal mb-3">BENEFITS</div>
                      <ul className="flex flex-col gap-2 mb-4">
                        {s.benefits.map((b) => (
                          <li key={b} className="flex items-start gap-2 text-ink-70 text-lg"><CheckCircle2 className="text-royal shrink-0 mt-1" size={16} />{b}</li>
                        ))}
                      </ul>
                    </div>
                    <div>
                      <div className="font-pixel text-10px text-royal mb-3">OUR PROCESS</div>
                      <div className="flex flex-wrap gap-2 mb-5">
                        {s.process.map((p, i) => <Chip key={p} tone="ink">{i + 1}. {p}</Chip>)}
                      </div>
                      <div className="font-pixel text-10px text-royal mb-3">TECH STACK</div>
                      <div className="flex flex-wrap gap-2 mb-5">
                        {s.tech.map((t) => <Chip key={t}>{t}</Chip>)}
                      </div>
